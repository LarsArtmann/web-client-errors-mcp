...
describe('BDD: Web Client Error Detection', () => {
  ...
  beforeEach(() => {
    ...
    testSessionId = sessionManager.createSession('https://example.com', {
      ...
      onlineStatus: true,
      domSnapshot: { exists: false, size: 0, timestamp: '2025-11-03T12:00:00.000Z' },
      networkConditions: {
        isOnline: true,
        connectionType: 'wifi',
        effectiveType: '4g'
  ...
  describe('Given a new error monitoring session', () => {
    ...
    describe('When multiple errors of the same type occur', () => {
      it('Then error patterns should be identified for analysis', () => {
        const errors = [
          errorDetectionService.createJavaScriptError('TypeError: Cannot read property foo', 'stack', 1, 1, 'https://example.com'),
          errorDetectionService.createJavaScriptError('TypeError: Cannot read property bar', 'stack', 2, 2, 'https://example.com'),
          errorDetectionService.createJavaScriptError('ReferenceError: x is not defined', 'stack', 3, 3, 'https://example.com')
        ];
      ...
      it('Then most common errors should be ranked', () => {
        const errors = [
          errorDetectionService.createJavaScriptError('TypeError: Cannot read property foo', 'stack', 1, 1, 'https://example.com'),
          errorDetectionService.createJavaScriptError('TypeError: Cannot read property bar', 'stack', 2, 2, 'https://example.com'),
          errorDetectionService.createJavaScriptError('TypeError: Cannot read property baz', 'stack', 3, 3, 'https://example.com'),
        ];
  ...
  describe('Given error detection with pattern matching', () => {
    ...
    it('Then pattern regex should match real errors', () => {
      const typeErrorPattern = errorDetectionService.getErrorPatterns()
        .find(p => p.name === 'TypeError: Undefined Property');
      
      expect(typeErrorPattern?.regex.test('TypeError: Cannot read properties of undefined')).toBe(true);
      expect(typeErrorPattern?.regex.test('TypeError: Cannot read property foo of null')).toBe(true);
    });
  });
